...
def build_system_prompt(window_summary: str, af_tools_present: bool) -> str:
    ...
    intent_map = (
        ...
        "\nElement / Matrix (chat.agentflow.website — главный социальный хаб владельца):\n"
        "  • Все Telegram аккаунты и соцсети уже подключены через bridges. НЕ открывай нативные приложения.\n"
        "  • «открой Element / посмотри Matrix / прочитай чаты» → firefox_open + firefox_navigate "
        "https://chat.agentflow.website → firefox_snapshot.\n"
        "  • «прочитай комнату X» → firefox_eval с селектором по aria-label / data-room-id.\n"
        "  • «отправь в Element / напиши в Matrix» → af_post_matrix_room(room_id, text) — это серверный "
        "путь через Dendrite, БЕЗ открывания Firefox. Если room_id неизвестен — открой Element и поищи.\n"
        "  • Получи список комнат через af_telegram_dialogs (TG-чаты появляются как Element-комнаты через "
        "bridge) или firefox_eval по DOM.\n"
        "  • «добавь акк X на платформу / подключи Y к AgentFlow / вытащи cookies для Z» →\n"
        "    1) firefox_get_cookies(domain=Z) — посмотри что есть в моём Firefox.\n"
        "    2) Открой соответствующую страницу AgentFlow для добавления (cabinet → integrations → Z).\n"
        "    3) firefox_export_cookies_to(domain=Z, dest_field_selector=форма для вставки). "
        "Native-confirm dialog покажет юзеру что вставляется.\n"
        "    4) Submit form. Verify success-toast в DOM.\n"
        "    5) af_remember(kind='lesson', tags=['onboarding', domain]) с тем что сработало.\n"
